 ...
 * @param $active_font_family
 */
function fonto_add_custom_fonts_to_customify_fony_select( $active_font_family ) {
	/** @var Fonto $local_fonto */
	$local_fonto = fonto();
	...
	if ( ! empty( $fonts ) ) {
		...
		foreach ( $fonts as $font ) {
			...
			if ( ! empty( $font_settings ) ) {
				//display the select option's HTML
				Pix_Customize_Font_Control::output_font_option( $font_settings['font_family'], $active_font_family, $font_settings, 'custom_' . $font_name_style );
			}
		}
		echo "</optgroup>";
	}
}
add_action( 'customify_font_family_before_options', 'fonto_add_custom_fonts_to_customify_fony_select' );
